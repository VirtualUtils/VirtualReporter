...
# By bwuch
function ShowWrongOS ([hashtable]$vCheckDataObjects) {
  ...
  if ($ShowWrongOS) {
    ...
    foreach ($vmguest in ($vCheckDataObjects["FullVM"] | where ({ $_.Guest.GuestFullname -ne $NULL -AND $_.Guest.GuestFullname -ne $_.Summary.Config.GuestFullName}))) {
