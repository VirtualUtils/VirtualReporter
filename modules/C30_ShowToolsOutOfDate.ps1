...
function ShowToolsOutOfDate ([hashtable]$vCheckDataObjects) {
  
  if ($Showtoolsupdate) {
    
    Write-CustomOut "..Checking VM Tools Out of Date"
    
    $UpdateTools = @($vCheckDataObjects["FullVM"] | Where {$_.Guest.ToolsStatus -eq "ToolsOld"} |Select Name, @{N="Status";E={$_.Guest.ToolsStatus}} | sort Name)
