# ---- VM Hardware Version ----
function ShowHardwareVersion ([hashtable]$vCheckDataObjects) {
  ...
  if ($ShowHWVer) {
    if ($vSphere -eq $true) {
      ...
      $HV = @($vCheckDataObjects["FullVM"] | Select Name, @{N="HardwareVersion";E={"Version $($_.Config.Version[5])"}} | Where {$_.HardwareVersion -ne "Version 7"} | sort name)
