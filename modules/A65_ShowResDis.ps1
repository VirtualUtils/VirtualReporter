...
function ShowResDis ([hashtable]$vCheckDataObjects) {
  
  if ($ShowResDis) {
    Write-CustomOut "..Checking Hosts Not responding or Disconnected"
    
    $RespondHosts = @($vCheckDataObjects["VMH"] | where {$_.ConnectionState -ne "Connected" -and $_.ConnectionState -ne "Maintenance"} | get-view | Select name, @{N="Connection State";E={$_.Runtime.ConnectionState}}, @{N="Power State";E={$_.Runtime.PowerState}})
