# ---- NTP Check ----
function ShowNTPCheck ([hashtable]$vCheckDataObjects) {
  
  if ($ShowNTP) {
  
    Write-CustomOut "..Checking NTP Name and Service"
    
    $NTPCheck = @($vCheckDataObjects["VMH"]| Where {$_.ConnectionState -ne "Disconnected"} | Select Name, @{N="NTPServer";E={$_ | Get-VMHostNtpServer}}, @{N="ServiceRunning";E={(Get-VmHostService -VMHost $_ | Where-Object {$_.key -eq "ntpd"}).Running}} | Where {$_.ServiceRunning -eq $false -or $_.NTPServer -notmatch $ntpserver})
