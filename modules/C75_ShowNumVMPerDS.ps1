...
function ShowNumVMPerDS ([hashtable]$vCheckDataObjects) {

  if ($ShowNumVMPerDS) {
   
    Write-CustomOut "..Checking Number of VMs per Datastore"

    $VMPerDS = @($vCheckDataObjects["Datastores"] | Select Name, @{N="NumVM";E={@($_ | Get-VM).Count}} | Where { $_.NumVM -gt $NumVMsPerDatastore} | Sort Name)
