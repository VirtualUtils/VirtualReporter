# ---- No VM Tools ----
function ShowVMsWithOutTools ([hashtable]$vCheckDataObjects) {
  
  if ($ShowTools){
    Write-CustomOut "..Checking VM Tools"
    $NoTools = @($vCheckDataObjects["FullVM"] | Where {$_.Runtime.Powerstate -eq "poweredOn" -And ($_.Guest.toolsStatus -eq "toolsNotInstalled" -Or $_.Guest.ToolsStatus -eq "toolsNotRunning")} | Select Name, @{N="Status";E={$_.Guest.ToolsStatus}} |sort Name)
