# ---- VC Errors ----
function ShowVIEvents ([hashtable]$vCheckDataObjects) {
  if ($ShowVIEvents) {
   
   Write-CustomOut "..Checking VI Events"

   $OutputErrors = @(Get-VIEvent -maxsamples 10000 -Start ($vCheckDataObjects["date"]).AddDays(-$VCEventAge ) -Type Error | Select @{N="Host";E={$_.host.name}}, createdTime, @{N="User";E={(Find-Username (($_.userName.split("\"))[1])).Properties.displayname}}, fullFormattedMessage)
